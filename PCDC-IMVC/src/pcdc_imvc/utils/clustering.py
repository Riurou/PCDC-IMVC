...
#def get_score(representation, Y_list, acc, nmi, ARI):
def get_score(data, labels, acc, nmi, ARI, fscore=None, init_centers=None):#----------------------------修改6------------------------------------
    # get clustering results and append them to list
    #----------------------------修改6------------------------------------
    y_preds, scores = Clustering(data, labels[0], init_centers=init_centers)
    #----------------------------修改6------------------------------------
    acc.append(scores['kmeans']['accuracy'])
    nmi.append(scores['kmeans']['NMI'])
    ARI.append(scores['kmeans']['ARI'])
    if fscore is not None:
        fscore.append(scores['kmeans']['F-score'])

    return scores


def Clustering(x_list, y, init_centers=None):#----------------------------修改6------------------------------------
    n_clusters = np.size(np.unique(y))

    x_final_concat = np.concatenate(x_list[:], axis=1)
    #----------------------------修改6------------------------------------
    init_args = {} if init_centers is not None else {'n_init': 10}
...
def clustering_metric(y_true, y_pred, decimals=4):
    """
    计算聚类指标：Accuracy、NMI、ARI、F-score

    Args:
        y_true: 真实标签 (n_samples,)
        y_pred: 聚类预测标签 (n_samples,)
        decimals: 保留小数位数

    Returns:
        dict: 包含 'accuracy', 'NMI', 'ARI', 'F-score'
    """
    # 调整聚类标签顺序，使其与真实标签尽量对应
    y_pred_adjusted = get_y_preds(y_true, y_pred, n_clusters=len(np.unique(y_true)))

    # Accuracy
    accuracy = np.round(np.mean(y_true == y_pred_adjusted), decimals)
    ...
    # F-score (macro-F1 on aligned labels)
    f_score = np.round(metrics.f1_score(y_true, y_pred_adjusted, average='macro'), decimals)

    return {'accuracy': accuracy, 'NMI': nmi, 'ARI': ari, 'F-score': f_score}, None

def get_cluster_sols(x, cluster_obj=None, ClusterClass=None, n_clusters=None, init_args={}, init_centers=None):#----------------------------修改6------------------------------------
    ...
    if cluster_obj is None:
        ...
        #----------------------------修改6------------------------------------
        if init_centers is not None and 'n_init' in init_args:
            init_args = {k: v for k, v in init_args.items() if k != 'n_init'}
        if init_centers is not None:
            cluster_obj = ClusterClass(n_clusters,
                                       init=init_centers,
                                       n_init=1,
                                       max_iter=300,
                                       **init_args)
